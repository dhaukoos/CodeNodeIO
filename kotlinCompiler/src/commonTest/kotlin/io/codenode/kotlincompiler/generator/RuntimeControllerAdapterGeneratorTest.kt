/*
 * RuntimeControllerAdapterGenerator Test
 * Tests for generating {Name}ControllerAdapter.kt from FlowGraph
 * License: Apache 2.0
 */
...
/**
 * Tests for RuntimeControllerAdapterGenerator - generates {Name}ControllerAdapter.kt
 * wrapping the Controller and implementing the Interface.
 */
class RuntimeControllerAdapterGeneratorTest {
    ...
    private val generator = RuntimeControllerAdapterGenerator()
    private val generatedPackage = "io.codenode.testapp.generated"

    // ========== Test 1: StopWatch-like flow ==========

    @Test
    fun `StopWatch-like flow generates adapter implementing interface`() {
        val flowGraph = createStopWatchLikeFlow()
        val result = generator.generate(flowGraph, generatedPackage)

        assertTrue(result.contains("class StopWatch2ControllerAdapter("))
        assertTrue(result.contains(") : StopWatch2ControllerInterface"))
    }

    @Test
    fun `StopWatch-like flow generates controller constructor param`() {
        val flowGraph = createStopWatchLikeFlow()
        val result = generator.generate(flowGraph, generatedPackage)

        assertTrue(result.contains("private val controller: StopWatch2Controller"))
    }
    ...
    @Test
    fun `StopWatch-like flow delegates observable state to controller`() {
        val flowGraph = createStopWatchLikeFlow()
        val result = generator.generate(flowGraph, generatedPackage)

        assertTrue(result.contains("override val seconds: StateFlow<Int>"))
        assertTrue(result.contains("get() = controller.seconds"))
        assertTrue(result.contains("override val minutes: StateFlow<Int>"))
        assertTrue(result.contains("get() = controller.minutes"))
    }

    @Test
    fun `StopWatch-like flow delegates executionState to controller`() {
        val flowGraph = createStopWatchLikeFlow()
        val result = generator.generate(flowGraph, generatedPackage)

        assertTrue(result.contains("override val executionState: StateFlow<ExecutionState>"))
        assertTrue(result.contains("get() = controller.executionState"))
    }
    ...
    fun `StopWatch-like flow delegates start method`() {
        ...
        assertTrue(result.contains("override fun start(): FlowGraph = controller.start()"))
    }
    ...
    fun `StopWatch-like flow delegates stop method`() {
        ...
        assertTrue(result.contains("override fun stop(): FlowGraph = controller.stop()"))
    }
    ...
    fun `StopWatch-like flow delegates reset method`() {
        ...
        assertTrue(result.contains("override fun reset(): FlowGraph = controller.reset()"))
    }
    ...
    fun `StopWatch-like flow delegates pause method`() {
        ...
        assertTrue(result.contains("override fun pause(): FlowGraph = controller.pause()"))
    }
    ...
    fun `StopWatch-like flow delegates resume method`() {
        ...
        assertTrue(result.contains("override fun resume(): FlowGraph = controller.resume()"))
    }
