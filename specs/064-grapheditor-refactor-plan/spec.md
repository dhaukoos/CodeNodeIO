# Feature Specification: GraphEditor Refactoring Plan

**Feature Branch**: `064-grapheditor-refactor-plan`
**Created**: 2026-04-02
**Status**: Draft
**Input**: User description: "GraphEditor Refactoring Plan — Define the migration map and characterization test suite for decomposing the graphEditor monolith into vertical-slice modules. Deliverables: (1) audit graphEditor and catalog every file by responsibility bucket, (2) write characterization tests that pin current behavior, (3) define module boundaries and extraction order, (4) create a meta-FlowGraph mapping the target architecture."

## User Scenarios & Testing

### User Story 1 - Audit and Catalog graphEditor Files (Priority: P1)

A developer preparing to refactor the graphEditor needs a complete inventory of what the module contains and how each file relates to user-facing workflows. They run the audit, which catalogs every file in `graphEditor/src` into vertical-slice responsibility buckets (Compose, Persist, Execute, Generate, Inspect) plus a "Composition Root" bucket for files that remain in graphEditor. The audit identifies cross-bucket dependencies (seams) that must be addressed during extraction.

**Why this priority**: Without a complete understanding of what graphEditor contains and how its parts relate, any extraction attempt risks breaking the application. The audit is the foundation for all subsequent work.

**Independent Test**: Run the audit tool/script on the current graphEditor source tree. Verify every `.kt` file in `graphEditor/src` appears in exactly one responsibility bucket. Verify cross-bucket dependencies are listed with source file, target file, and dependency type (function call, type reference, import).

**Acceptance Scenarios**:

1. **Given** the graphEditor module exists with its current source files, **When** the audit is executed, **Then** every `.kt` file in `graphEditor/src/jvmMain/kotlin/` is assigned to exactly one responsibility bucket.
...
4. **Given** the audit output exists, **When** the total file count is compared against the actual file count in `graphEditor/src`, **Then** they match exactly — no files are missing or duplicated.

---

### User Story 2 - Write Characterization Tests Pinning Current Behavior (Priority: P1)

A developer needs confidence that extracting code from graphEditor will not change observable behavior. They write characterization tests that capture what the code does right now — not what it should do. These tests cover graph data operations (add node, connect ports, validate, serialize), execution/runtime (create graph, run, assert state transitions), and ViewModel integration (verify ViewModel exposes correct state after graph mutations). All characterization tests pass on the current codebase before any extraction begins.
...
**Independent Test**: Run `./gradlew :graphEditor:jvmTest` and verify all characterization tests pass. Intentionally break a seam (e.g., change a serialization format) and verify at least one characterization test fails — confirming the tests actually pin behavior.
...
4. **Given** all characterization tests are written, **When** `./gradlew :graphEditor:jvmTest` is run on the unmodified codebase, **Then** all characterization tests pass.
5. **Given** all characterization tests pass, **When** a known seam is intentionally modified (e.g., a serialization output format), **Then** at least one characterization test fails — confirming the tests detect behavioral changes.

---

### User Story 3 - Define Module Boundaries and Extraction Order (Priority: P2)

A developer needs a concrete plan for which code moves where and in what order. Using the audit results, they define five vertical-slice modules (flowGraph-compose, flowGraph-persist, flowGraph-execute, flowGraph-generate, flowGraph-inspect), assign each audited file to its target module, specify the public API (Kotlin interfaces) for each module, and determine the extraction order based on dependency analysis. The extraction order ensures that each step leaves the application fully functional.
...
1. **Given** the audit is complete, **When** module boundaries are defined, **Then** every file in the audit is assigned to exactly one target module: flowGraph-compose, flowGraph-persist, flowGraph-execute, flowGraph-generate, flowGraph-inspect, or graphEditor (composition root).
2. **Given** module boundaries are defined, **When** the public API for each module is specified, **Then** each module has a set of Kotlin interface definitions that represent the functions graphEditor currently calls internally for that module's responsibility.
...
- What happens when a graphEditor file belongs to multiple responsibility buckets? It is assigned to its primary bucket, with cross-references noted. During extraction, the file may need to be split.
...
- What happens when a file in graphEditor has no clear bucket assignment? It is temporarily assigned to "Composition Root" (stays in graphEditor) and flagged for review during the extraction of adjacent modules.
...
- **FR-001**: The audit MUST catalog every `.kt` file in `graphEditor/src/jvmMain/kotlin/` with its responsibility bucket assignment.
- **FR-002**: The audit MUST identify all cross-bucket dependencies with source file, target file, and dependency type.
...
- **FR-004**: Characterization tests MUST cover runtime execution: graph creation, execution start/stop, and state transition assertions.
...
- **FR-006**: All characterization tests MUST pass on the unmodified graphEditor codebase.
...
- **Responsibility Bucket**: A classification category for graphEditor files, corresponding to a target vertical-slice module or the composition root.
...
- **SC-001**: 100% of `.kt` files in `graphEditor/src/jvmMain/kotlin/` are cataloged in the audit with a bucket assignment — zero files missing.
- **SC-002**: All cross-bucket dependencies are documented with sufficient detail (source, target, type) to plan interface boundaries.
...
- **SC-004**: All characterization tests pass on the current unmodified codebase via `./gradlew :graphEditor:jvmTest`.
...
- The existing graphEditor codebase is stable enough to write characterization tests against — no major in-flight refactors that would invalidate the tests.
