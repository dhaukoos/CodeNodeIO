# Research: GraphEditor Refactoring Plan
...
**Decision**: Decompose graphEditor into five workflow-oriented modules plus the composition root.
...
| flowGraph-execute | Running and observing | Dynamic runtime pipeline, coroutine orchestration, execution control (pause/resume/step), data flow animation state. Input: FlowGraph. Output: execution state stream, IP snapshots. |
| flowGraph-generate | Producing deployable code | Module save workflow, CodeNode definition codegen, runtime file generation, build configuration. Input: FlowGraph + generation config. Output: file tree on disk. |
...
**Decision**: Use a multi-signal approach combining import analysis, type usage, and call-site patterns to assign each of the 77 source files to exactly one bucket.
...
- **Automated static analysis tool**: Could parse the AST, but overkill for 77 files. Manual analysis with import checking is sufficient and produces richer annotations (why a file belongs to a bucket, not just that it does).

---

## R3: Characterization Test Strategy

**Decision**: Write characterization tests at the ViewModel/state layer, organized into four test classes covering the major seam categories.

| Test Class | Seam Category | What It Pins |
|-----------|---------------|-------------|
| GraphDataOpsCharacterizationTest | Graph data operations | Node creation, port connection, port type validation, cycle detection, GraphNode creation, port mapping |
| RuntimeExecutionCharacterizationTest | Runtime execution | Graph execution start/stop, state transitions, channel wiring, execution state propagation |
| ViewModelCharacterizationTest | ViewModel integration | ViewModel state after graph mutations, palette state management, properties panel state |
| SerializationRoundTripCharacterizationTest | Serialization | .flow.kts round-trip fidelity for all node types (CodeNode, GraphNode, nested GraphNode), port types, connections |
...
- Tests at this layer run without Compose — just kotlin.test + coroutines-test
- Existing test patterns in graphEditor/src/jvmTest already demonstrate this approach (e.g., GraphEditorViewModelTest, GraphStateTest)
- Four categories map to the four types of seams the audit will identify
...
3. **flowGraph-execute** (third): Runtime pipeline has clear boundaries (FlowGraph in, execution state out) but has more integration points with the UI (animation state, preview panel).
4. **flowGraph-generate** (fourth): Code generation depends on persist (for .flow.kts output) and potentially inspect (for node definitions). Extract after its dependencies are stable.
