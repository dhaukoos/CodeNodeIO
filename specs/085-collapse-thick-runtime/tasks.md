...
- [ ] T021a [US1] In `/Users/dhaukoos/CodeNodeIO/flowGraph-generate/src/commonMain/kotlin/io/codenode/flowgraphgenerate/generator/ModuleGenerator.kt`, locate the per-module rendering entry point (the existing `ModuleSpec`-consuming method) and add a private helper `fun toModuleRuntimeSpec(spec: ModuleSpec): ModuleRuntimeSpec` that maps module name, package, controller-interface FQCN, state-object FQCN, node-registry entries, and typed state flows. Verify by adding a unit test in `ModuleGeneratorTest.kt` (or equivalent) covering the StopWatch and Addresses spec shapes
- [ ] T021b [US1] In same `ModuleGenerator.kt`, add a call to `ModuleRuntimeGenerator().generate(toModuleRuntimeSpec(spec))` and write the result to `{moduleRoot}/src/commonMain/kotlin/{packagePath}/{Module}Runtime.kt` within the existing per-module rendering loop. Idempotent: skip write if existing file content matches
- [ ] T021c [US1] In same `ModuleGenerator.kt`, remove every call site that invokes `RuntimeControllerGenerator`, `RuntimeControllerAdapterGenerator`, or `RuntimeFlowGenerator`. Remove their import lines. Run `./gradlew :flowGraph-generate:compileKotlinJvm` and confirm clean compile (the deletions of the generator classes themselves happen in Phase 7)
- [ ] T021d [US1] In same `ModuleGenerator.kt`, add scheduled deletion of `controller/{Module}Controller.kt`, `controller/{Module}ControllerAdapter.kt`, and `flow/{Module}Flow.kt` for every module being regenerated. Each deletion records a `FileChange { kind = DELETED }` entry in the structured summary. (Safety check is implemented in T022.)
