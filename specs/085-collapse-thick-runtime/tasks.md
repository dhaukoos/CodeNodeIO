...
- [ ] T024 [P] [US1] Regenerate `StopWatch` — **DEFERRED**: requires GraphEditor-mediated regeneration (canonical path per `ModuleSaveService.saveModule` deprecation pointing to "Code Generator panel"). Doing this programmatically would require building substantial fixture infrastructure (loading `.flow.kt`, populating `_codeNodeClass` configuration, etc.) that the live system handles automatically. Defer to follow-up session combined with US2 (T030–T038) per FR-014 atomic-landing requirement
- [ ] T025 [P] [US1] Regenerate `Addresses` — **DEFERRED** (same reason as T024)
- [ ] T026 [P] [US1] Regenerate `UserProfiles` — **DEFERRED** (same reason as T024)
- [ ] T027 [P] [US1] Regenerate `EdgeArtFilter` — **DEFERRED** (same reason as T024)
- [ ] T028 [P] [US1] Regenerate `WeatherForecast` — **DEFERRED** (same reason as T024)
- [ ] T029 [US1] Verify SC-001 line-count drop — **DEFERRED** until T024–T028 complete
