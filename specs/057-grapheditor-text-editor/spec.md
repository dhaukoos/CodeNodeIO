...
- What happens when the user selects a node that was not generated by the Node Generator (e.g., a built-in node with no editable source file)? The pencil icon is disabled or hidden, and the node does not appear in the dropdown's file list.
...
- **FR-012**: The edit button MUST NOT appear for nodes that have no editable source file (built-in nodes).
...
- **CodeNode Source File**: A generated source file on disk associated with a CodeNode in the flowGraph. Key attributes: file path, file content, read/write status, associated node name.
...
- **SC-003**: 100% of CodeNode source files generated by the Node Generator are accessible for editing through the graphEditor.
...
- Only files generated by the Node Generator (at MODULE, PROJECT, or UNIVERSAL placement levels) are editable. Built-in nodes and non-file-backed nodes are excluded.
