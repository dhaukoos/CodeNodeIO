...
- [ ] T012 [US1] Add "Palette" section to the GraphNode properties view in `PropertiesPanel.kt`: when a GraphNode is selected, render a level selector dropdown (`PlacementLevel.availableLevels(moduleLoaded)`) and an "Add to Palette" button below the existing child nodes section in `graphEditor/src/jvmMain/kotlin/ui/PropertiesPanel.kt`
- [ ] T013 [US1] Add save action to `PropertiesPanelViewModel`: implement `saveGraphNodeToPalette(graphNode: GraphNode, level: PlacementLevel)` that calls `GraphNodeTemplateRegistry.saveGraphNode()` and updates palette state in `graphEditor/src/jvmMain/kotlin/viewmodel/PropertiesPanelViewModel.kt`
- [ ] T014 [US1] Create `GraphNodePaletteSection` composable that renders a collapsible "GraphNodes" dropdown in the Node Palette, listing all `GraphNodeTemplateMeta` entries from the registry as cards with name and port count badges (green "N in", blue "M out") in `graphEditor/src/jvmMain/kotlin/ui/GraphNodePaletteSection.kt`
- [ ] T015 [US1] Integrate `GraphNodePaletteSection` into `NodePalette.kt`: add the "GraphNodes" section above or below the existing CodeNodeType category sections, passing the registry's template list in `graphEditor/src/jvmMain/kotlin/ui/NodePalette.kt`
- [ ] T016 [US1] Implement instantiation logic: create `GraphNodeTemplateInstantiator` with `instantiate(meta: GraphNodeTemplateMeta, dropPosition: Offset): GraphNode` that loads the full template via `GraphNodeTemplateSerializer.loadTemplate()`, generates fresh IDs for the GraphNode and all child nodes, remaps all internal connection source/target IDs, remaps all port `owningNodeId` references, remaps all `portMapping.childNodeId` references, and sets the drop position in `graphEditor/src/jvmMain/kotlin/io/codenode/grapheditor/state/GraphNodeTemplateInstantiator.kt`
- [ ] T017 [US1] Add drag-and-drop support for GraphNode palette cards: apply `draggableNodeType`-equivalent modifier to `GraphNodePaletteSection` cards, and extend the canvas drop handler to detect GraphNode template drops and call `GraphNodeTemplateInstantiator.instantiate()` then `graphState.addNode()` in `graphEditor/src/jvmMain/kotlin/ui/DragAndDropHandler.kt` and `graphEditor/src/jvmMain/kotlin/ui/GraphNodePaletteSection.kt`
- [ ] T018 [US1] **MANUAL** Verify quickstart Scenario 1: save a GraphNode to the palette at Project level, verify `.flow.kts` file created at `{projectRoot}/graphnodes/`, verify card appears in palette
- [ ] T019 [US1] **MANUAL** Verify quickstart Scenario 2: drag saved GraphNode from palette to canvas, verify independent copy with unique IDs, verify child nodes and connections intact
- [ ] T020 [US1] **MANUAL** Verify quickstart Scenario 3: restart graphEditor, verify saved GraphNode persists in palette, drag to canvas and verify functional
