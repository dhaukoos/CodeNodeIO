...
**Created**: 2026-04-26
**Status**: Ready to resume — feature 085 (universal-runtime collapse) shipped 2026-04-28; re-run `/speckit.clarify` then `/speckit.plan` against the new universal runtime before resuming implementation.
...
> **RESUMPTION NOTICE (2026-04-28)**: Feature 085 (universal-runtime collapse) has shipped. The thick stack that this spec's plan / research / data-model / contracts / quickstart targeted (`{Module}Controller.kt`, `{Module}ControllerAdapter.kt`, `{Module}Flow.kt` runtime, `UIFBPSpecAdapter`, the eliminated entity-module generators) is gone — across all modules, generation now produces a single `controller/{Module}Runtime.kt` factory function `create{Module}Runtime(flowGraph): {Module}ControllerInterface` whose return type extends `ModuleController` (the universal control surface in `fbpDsl`). The Runtime Preview path uses a Java reflection proxy in `ModuleSessionFactory` that already handles `getStatus(): FlowExecutionStatus` and the typed state-flow getters declared by `{Module}ControllerInterface`. Before resuming implementation: (1) re-run `/speckit.clarify` on this spec to retire the thick-stack requirements (FR-005a, SC-008, deployability framing); (2) re-run `/speckit.plan` to regenerate plan / research / data-model / contracts / quickstart against the universal runtime — anchor the new plan on the contracts in `specs/085-collapse-thick-runtime/contracts/` and on the production-app integration template in `specs/085-collapse-thick-runtime/quickstart.md` (VS-D5). The user-facing spec body (US1–US4, most FRs, most SCs) needs only minor edits — the gap-analysis framing remains valid; what's changed is the generator surface UI-FBP needs to fit into.
...
- Q: How should the obsolete thick-stack mandate (FR-005a) and its references in FR-005 be reconciled with feature 085's universal-runtime collapse? → A: Retire FR-005a entirely; rewrite FR-005's enumeration to "preview-registration entry point, controller interface (extending `ModuleController`), and a `create{Module}Runtime` factory" — the post-085 universal set.
- Q: How should SC-008's obsolete file-comparison framing be reconciled with the post-085 reality? → A: Retire SC-008 entirely; deployability parity is now implicit in FR-005's universal-set wording, and feature 085's `quickstart.md` VS-D5 production-app integration template is the canonical evidence.
- Q: Is the inherited assumption that `moduleName == ComposableName == flowGraphName` (i.e., the prefix for every generated file is derived from the *module* name) still correct after features 082 and 083? → A: No — features 082 and 083 made multiple flow graphs per module possible. The prefix for every UI-FBP-generated file MUST now be derived from the *flow graph's* prefix name, not the module name, and the Composable function name is the user-authored name from the qualifying UI source file (independent of either the module or the flow graph name). FR-005 and the Key Entities ("Preview Registration Entry Point", file naming) are updated accordingly. **Implication for the inherited `PreviewProviderGenerator` (feature 085)**: it currently reuses `flowGraph.name.pascalCase()` for both the `PreviewRegistry` key (correct) AND the Composable function call (potentially wrong when the composable name diverges from the flow graph name). Feature 084 must surface this seam — either by extending the 085 generator to accept a separate composable-name input, or by emitting a UI-FBP-specific PreviewProvider that uses the parser-extracted Composable name for the function call while keeping the flow-graph-prefix as the registry key.
- Q: With feature 085's `ModuleGenerator` scaffolding now emitting `jvm()` target + `preview-api` dep for every module created via the Generate Module path, what is UI-FBP's remaining responsibility for build wiring (FR-009)? → A: Retire FR-009. Build wiring is fully delegated to module scaffolding; UI-FBP assumes a scaffolded module and refuses to operate on a non-scaffolded one (legacy modules like the current `TestModule` require a one-time manual or scripted migration before UI-FBP works against them). Document the migration path in `quickstart.md`.
- Q: After features 082/083 made multiple flow graphs per module the norm, what is UI-FBP's multi-UI posture (FR-017)? → A: UI-FBP runs against an explicit `{flow graph, UI file}` pair supplied by the user, mirroring feature 085's Generate Module file-selector pattern. The user picks both inputs explicitly; UI-FBP does not implicitly scan the module to discover flow graphs or qualifying UI files. This removes ambiguity: a module may host any number of flow graphs and any number of UI files, but each UI-FBP run is unambiguous because both inputs are explicit. FR-017's "refuse on more than one qualifying UI file" condition is retired in favor of this pair-based input model.
...
- **Module contains multiple qualifying UI files or multiple flow graphs**: Per features 082/083, this is now the norm rather than an edge case. UI-FBP avoids ambiguity by taking an explicit `{flow graph, UI file}` pair as input (user picks both via the GraphEditor's file selectors, mirroring feature 085's Generate Module pattern). Generated artifacts are prefixed by the flow graph's prefix name, so multiple runs against the same module produce non-colliding outputs.
...
- **FR-005**: UI-FBP code generation MUST emit, in addition to today's State, ViewModel, Source CodeNode, and Sink CodeNode files, every additional artifact required for both (a) Runtime Preview to operate end-to-end and (b) the module to be deployable to a production Android/iOS/Desktop app without the GraphEditor in the loop. The set of "every additional artifact" is, post-feature-085, exactly: a **preview-registration entry point** (`{FlowGraph}PreviewProvider.kt` in `jvmMain/userInterface/`), a **controller interface** (`{FlowGraph}ControllerInterface.kt` extending `io.codenode.fbpdsl.runtime.ModuleController`), and a **Runtime factory** (`{FlowGraph}Runtime.kt` exposing `create{FlowGraph}Runtime(flowGraph): {FlowGraph}ControllerInterface`). The `{FlowGraph}` prefix is the flow graph's prefix name (post-082/083 a module may host more than one flow graph; the prefix MUST be derived from the flow graph, not the module). The PreviewProvider's `PreviewRegistry` key MUST equal the flow graph prefix (matches what `RuntimePreviewPanel` looks up); its body MUST invoke the user-authored Composable by its actual function name from the qualifying UI source file (which may or may not coincide with the flow graph prefix). UI-FBP modules ride the universal runtime collapse delivered by feature 085 and share this shape with every other module in the project.
...
- **FR-008**: The generated module MUST compile cleanly under the project's standard build invocation, with no manual edits to source files, package declarations, or imports — given that the host module was created by feature 085's module scaffolding (which already includes the `jvm()` target and the `io.codenode:preview-api` dependency in `jvmMain`).
- **FR-009**: Build wiring (the `jvm()` target and the `preview-api` dependency that UI-FBP-generated artifacts require) is the responsibility of module scaffolding (feature 085's `ModuleGenerator`), not UI-FBP. UI-FBP MUST detect a non-scaffolded host module before emitting any output and refuse with a clear, actionable error directing the user to a one-time migration (documented in `quickstart.md`). UI-FBP MUST NOT silently mutate the host module's `build.gradle.kts`.
...
- **FR-014**: Generation MUST operate against an existing module — it is not responsible for creating a new module from scratch (module scaffolding is handled separately by the module-creation flow). The user's expectation is that they have already created the module, dropped a qualifying UI file into it, and are now running UI-FBP against an explicit `{flow graph, UI file}` pair.
- **FR-015**: The generator MUST take its inputs as an explicit `{flow graph (`.flow.kt`), qualifying UI file}` pair supplied by the user (via GraphEditor file selectors mirroring feature 085's Generate Module pattern). The generator MUST validate that the UI file qualifies as a UI-FBP source (Composable function with a `viewModel` parameter) before emitting any output and MUST refuse with a clear, actionable error if it does not. The generator MUST NOT implicitly scan the host module to discover flow graphs or qualifying UI files; ambiguity is removed by the explicit-pair input.
...
- **Preview Registration Entry Point**: A generated artifact (`{FlowGraph}PreviewProvider.kt` in `jvmMain/userInterface/`) that registers an entry under the **flow graph's prefix name** (the key `RuntimePreviewPanel` looks up) and renders the user-authored Composable function from the qualifying UI source file. Post-082/083, the registry key, generated-file prefix, and Composable function name are three potentially-distinct identifiers.
- **Runtime Controller Contract**: A generated interface (`{FlowGraph}ControllerInterface` extending `io.codenode.fbpdsl.runtime.ModuleController`) plus its two implementers — (a) the reflection proxy synthesized by `ModuleSessionFactory.createControllerProxy` for the GraphEditor's Runtime Preview path, and (b) the anonymous `object : {FlowGraph}ControllerInterface, ModuleController by controller { ... }` returned by the generated `create{FlowGraph}Runtime(flowGraph)` factory for the production-app deployment path. Post-feature-085 there is no separate per-module `{Module}Controller` class.
- **FlowGraph DSL Value**: The `FlowGraph` DSL value loaded from the user's `.flow.kt` file and passed into `create{FlowGraph}Runtime(flowGraph)` (or into `ModuleSessionFactory.createSession(...)` for Runtime Preview). `DynamicPipelineController` consumes this value directly — there is no separate generated "Runtime Flow" class post-feature-085.
...
- The reference for "what a deployable production app expects to find" is the **post-feature-085 universal artifact set**: `{Module}ControllerInterface` (extending `io.codenode.fbpdsl.runtime.ModuleController`) + `{Module}Runtime` factory (`create{Module}Runtime(flowGraph): {Module}ControllerInterface`) + `{Module}PreviewProvider` (jvmMain) + ViewModel + State + nodes. Every entity module in the DemoProject (StopWatch, UserProfiles, Addresses, EdgeArtFilter, WeatherForecast) was migrated to this shape during feature 085, and feature 085's `quickstart.md` VS-D5 documents the canonical production-app integration template. UI-FBP modules ride this same shape with no UI-FBP-specific consumer code paths. The pre-feature-085 thick stack (`Controller.kt` + `ControllerAdapter.kt` + `Flow.kt` runtime) is gone across all modules.
...
- **SC-006**: The current `TestModule` (with `DemoUI.kt`) is brought to a state where it loads in Runtime Preview, after applying the documented one-time migration step (Quickstart VS-A1: build wiring + legacy `saved/` cleanup + import-path fix) followed by a single re-run of UI-FBP code generation. After that point, no duplicate State/ViewModel files (or other legacy artifacts) remain.
