...
- [X] T014 Handle null RuntimeSession in `RuntimePreviewPanel` in `graphEditor/src/jvmMain/kotlin/ui/RuntimePreviewPanel.kt` — if `runtimeSession` is null (unknown module), disable all execution controls and show "No runtime available for this module" in the preview area. Update the parameter type to `RuntimeSession?` if needed and guard all control button `onClick` handlers.
- [X] T015 Run quickstart.md end-to-end validation — follow the manual test flow in `specs/040-generalize-runtime-preview/quickstart.md`: launch graphEditor, load StopWatch, verify preview works, load UserProfiles, verify preview works, switch between modules, verify clean transitions.
