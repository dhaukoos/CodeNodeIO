...
- [ ] T052 [US3] Manually execute Quickstart `VS-B1`, `VS-B2`, and `VS-B3` to verify the recompile UI works uniformly across all three placement tiers and that compile errors surface with file/line information sufficient to locate the offending source without external tools (SC-003 / SC-006). (Deferred — requires hands-on session against TestModule + a Project-tier node + a Universal-tier node.)
