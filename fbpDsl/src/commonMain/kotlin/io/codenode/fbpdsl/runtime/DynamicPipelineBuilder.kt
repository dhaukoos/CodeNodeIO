            ...
            if (sourceDef != null) {
                ...
                if (outputIndex < 0) {
                    ...
                    errors.add(ValidationError(
                        ...
                        message = "Output port '$portName' not found on canvas node '${sourceDef.name}'"
                    ))
                } else if (outputIndex >= sourceDef.outputPorts.size) {
                    val portName = extractPortName(conn.sourcePortId, conn.sourceNodeId)
                    errors.add(ValidationError(
                        type = ValidationErrorType.PORT_COUNT_MISMATCH,
                        nodeId = conn.sourceNodeId,
                        nodeName = sourceDef.name,
                        message = "Output port '$portName' (index $outputIndex) exceeds '${sourceDef.name}' definition (has ${sourceDef.outputPorts.size} output ports)"
                    ))
                }
            }
            ...
            if (targetDef != null) {
                ...
                if (inputIndex < 0) {
                    ...
                    errors.add(ValidationError(
                        ...
                        message = "Input port '$portName' not found on canvas node '${targetDef.name}'"
                    ))
                } else if (inputIndex >= targetDef.inputPorts.size) {
                    val portName = extractPortName(conn.targetPortId, conn.targetNodeId)
                    errors.add(ValidationError(
                        type = ValidationErrorType.PORT_COUNT_MISMATCH,
                        nodeId = conn.targetNodeId,
                        nodeName = targetDef.name,
                        message = "Input port '$portName' (index $inputIndex) exceeds '${targetDef.name}' definition (has ${targetDef.inputPorts.size} input ports)"
                    ))
                }
            }
        }
